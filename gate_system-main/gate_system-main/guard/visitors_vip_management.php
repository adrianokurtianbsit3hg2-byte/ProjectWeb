...
<div class="visitor-container">
    <form id="visitorForm" class="entry-form">
        ...
        <div class="form-group">
            ...
            <textarea id="reason" name="reason" placeholder="Brief reason" required></textarea>
        </div>
        <button type="submit" class="submit-btn">
            <i class="fas fa-check-circle"></i> Submit Visitor Entry
        </button>
    </form>
...
document.getElementById('visitorForm').addEventListener('submit', function(e) {
    ...
    fetch('../php/action_add__visitor_entries.php', { method: 'POST', body: formData })
        .then(r => r.json())
        .then(data => {
            if (data.success) {
...
function showSuccessMessage(data) {
    ...
    responseCard.innerHTML = `
        <div class="response-header">
            ...
            <span>${isTimeIn ? 'VISITOR TIME IN RECORDED' : 'VISITOR TIME OUT RECORDED'}</span>
        </div>
...
function loadActiveVisitors() {
    ...
    fetch('../php/action_list__visitor_active.php')
        .then(r => r.json())
        .then(data => {
            if (!data.success) {
            ...
            if (!data.active || data.active.length === 0) {
                tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; color:#666;">No active visitors.</td></tr>';
                return;
            }
            tbody.innerHTML = data.active.map(v => `
                <tr>
                    ...
                    <td>${v.guard_in || '-'}</td>
                    <td>
                        <button class="logout-btn" onclick="logoutVisitor('${v.entry_key}')">
...
function logoutVisitor(entryKey) {
    const responseCard = document.getElementById('responseCard');
    responseCard.className = 'response-card';
    responseCard.style.display = 'block';
    ...
    fetch('../php/action_logout__visitor.php', { method: 'POST', body: formData })
        .then(r => r.json())
        .then(data => {
            if (data.success) {
                showSuccessMessage(data);
...
loadActiveVisitors();
document.getElementById('refreshList').addEventListener('click', loadActiveVisitors);
</script>